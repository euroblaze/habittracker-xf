...
export default function PlayerHabitTracker() {
  const params = useParams()
  const router = useRouter()
  const playername = params.playername as string

  // Validate player name and capitalize properly
  const getValidPlayerName = (name: string) => {
    const lowerName = name.toLowerCase()
    if (lowerName === "doro") return "Doro"
    if (lowerName === "ashant") return "Ashant"
    return null
  }

  const currentPartner = getValidPlayerName(playername)
  ...
  // Redirect if invalid player name
  useEffect(() => {
    if (!currentPartner) {
      router.push("/")
      return
    }
  }, [currentPartner, router])
  ...
  // Don't render if invalid player
  if (!currentPartner) {
    return null
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-6">
          <p className="text-sm text-gray-500">
            Your personal habit tracker: <span className="font-mono">/{playername.toLowerCase()}</span>
          </p>
        </div>
